================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: DdeClientTestApp プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した DdeClientTestApp には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには DdeClientTestApp アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

DdeClientTestApp.vcproj
...
DdeClientTestApp.h
   ...
   CDdeClientTestAppApp アプリケーション クラスを宣言します。

DdeClientTestApp.cpp
   これは、アプリケーション クラス CDdeClientTestAppApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\DdeClientTestApp.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル DdeClientTestApp.rc に含まれていま
   す。

res\DdeClientTestApp.rc2
...
DdeClientTestAppDlg.h, DdeClientTestAppDlg.cpp - ダイアログ
   これらのファイルは CDdeClientTestAppDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な DdeClientTestApp.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   DdeClientTestApp.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
